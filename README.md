...
- **Detail window** — a separate, non-modal **record editor** where fields are laid out **vertically** (one control per column, driven by the live table schema). You can **reorder the field list** for easier editing: sort by **Seq** (table column order), **Field Name** (alphabetical), or **Value** (current text in each editor, with NULL-checked fields grouped). Nullable columns include an explicit **NULL** option so you can clear values where SQLite allows it.
